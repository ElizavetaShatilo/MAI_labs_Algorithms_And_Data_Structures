    ...
    for (int i = 0; i < in_n; i++) {
        ...
        if (cur.type == OPERATOR) {
            ...
            while (top >= 0 && stack[top].type == OPERATOR) {
                ...
                } else if ((cur.op == '~' && p2 < p1) || (cur.op != '~' && p2 <= p1)) {
                    out[j++] = stack[top--];
                } else {
                    break;
                }
...
int isSum(Node *node) {
    return node != NULL && node->op == '+';
}

void transformPower(Node *node) {
    if (node == NULL)
        return;

    transformPower(node->left);
    transformPower(node->right);

    if (node->op == '^') {
        if (isSum(node->right)) {
            Node *base = node->left;
            Node *sumLeft = node->right->left;
            Node *sumRight = node->right->right;

            Node *power1 = createOp('^', copyTree(base), copyTree(sumLeft));
            Node *power2 = createOp('^', copyTree(base), copyTree(sumRight));
            Node *product = createOp('*', power1, power2);

            node->op = product->op;
            node->left = product->left;
            node->right = product->right;
            node->value = NULL;
...
Node *copyTree(Node *node) {
    ...
    Node *newNode = createOp(node->op, copyTree(node->left), copyTree(node->right));
    return newNode;
}
